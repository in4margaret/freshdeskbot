...
const createTicket = require('./freshdesk.js').createTicket;
...
bot.dialog('SubmitTicket', [
    ...
    (session, result, next) => {
        ...
        if (!session.dialogData.email) {
            builder.Prompts.text(session, 'Please provide your e-mail');
        } else {
            next();
        }
    },
    (session, result, next) => {
        ...
        var message = `Great! I'm going to create a "${session.dialogData.severity}" severity ticket in the "${session.dialogData.category}" category. ` +
                      `The description I will use is "${session.dialogData.description}"`+
                      `Your email is "${session.dialogData.email}". Can you please confirm that this information is correct?`;

        builder.Prompts.confirm(session, message, { listStyle: builder.ListStyle.button });
    },
    (session, result, next) => {
        if (result.response) {
            var severityOptions = { "high": 3, "normal": 2, "low": 1 };
            var data = {
                description: session.dialogData.description,
                subject: session.dialogData.category,
                email: session.dialogData.email,
                priority: severityOptions[session.dialogData.severity],
            };
            var dataForCard = {
            ...
            args.result.value.forEach((faq, i) => {
                msg.addAttachment(
                    new builder.ThumbnailCard(session)
                        ...
                        .images([builder.CardImage.create(session, 'https://raw.githubusercontent.com/in4margaret/freshdeskbot/master/assets/head-smiling-medium.png')])
        ...
        analyzeText(answer, (err, score) => {
            if (err) {
                ...
                if (score < 0.5) {
                    ...
                    cardImageUrl = 'https://raw.githubusercontent.com/in4margaret/freshdeskbot/master/assets/head-sad-medium.png';
                } else {
                    cardText = 'Thanks for sharing your experience.';
                    cardImageUrl = 'https://raw.githubusercontent.com/in4margaret/freshdeskbot/master/assets/head-smiling-medium.png';
                }
